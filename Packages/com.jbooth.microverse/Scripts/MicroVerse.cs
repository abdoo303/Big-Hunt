...
namespace JBooth.MicroVerseCore
{
    ...
    public partial class MicroVerse : MonoBehaviour
    {
        ...
        /// maps.
        /// </summary>
        public void SaveBackToTerrain(bool forceFinishSpawnProcssing = false, bool saveHoles = true)
        {
            ...
            foreach (var terrain in terrains)
            {
                ...
                if (needHoleSync && (saveHoles || options.settings.SyncHolesOnSave))
                {
        ...
        float[] indexRemap = new float[256];
        ...
        { 
            ...
            {
                indexRemapBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, 256, 4);
            }

            if (GetMegasplatTexture(terrains[0]) != null)
            {
                alphaSeamShader.EnableKeyword("_MEGASPLAT");
            }
            else
            {
                alphaSeamShader.DisableKeyword("_MEGASPLAT");
            }
            foreach (var terrain in terrains)
            {
        ...
        public void Modify(bool writeToCPU = false, bool noAsync = false, bool boundsCull = false)
        {
            ...
            Texture2D mega = GetMegasplatTexture(terrains[0]);
            if (mega != null)
            {
                splatMapRes = mega.width;
            }
        ...
        static ComputeShader rasterToMega = null;
        void RasterizeSplatMaps(Terrain terrain, RenderTexture indexMap, RenderTexture weightMap, bool writeToCPU)
        {
            ...
            Texture2D megaTex = GetMegasplatTexture(terrain);
            if (megaTex != null)
            {
#if UNITY_EDITOR

                RenderTextureDescriptor desc = new RenderTextureDescriptor(megaTex.width, megaTex.height,
                ...
                if (rasterToMega == null)
                {
                    rasterToMega = (ComputeShader)Resources.Load("MicroVerseRasterToMega");
                }
                int kernelHandle = rasterToMega.FindKernel("CSMain");
                rasterToMega.SetTexture(kernelHandle, "_WeightMap", weightMap);
                rasterToMega.SetTexture(kernelHandle, "_IndexMap", indexMap);
                rasterToMega.SetTexture(kernelHandle, "_Result0", rt);
                rasterToMega.Dispatch(kernelHandle, Mathf.CeilToInt((float)megaTex.width / 8), Mathf.CeilToInt((float)megaTex.height / 8), 1);
                RenderTexture.active = rt;
                megaTex.ReadPixels(new Rect(0, 0, rt.width, rt.height), 0, 0);
                megaTex.Apply();
                var tga = megaTex.EncodeToTGA();
                var path = UnityEditor.AssetDatabase.GetAssetPath(megaTex);
                System.IO.File.WriteAllBytesAsync(path, tga);
                RenderTexture.active = null;
                return;
                
#endif
            }
            else // write to unity terrain
            {
